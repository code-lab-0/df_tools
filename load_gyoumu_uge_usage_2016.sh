...
do
    if [ -r parsed_data/gyoumu_phase1_2016${m}.tsv ]; then
        for queue in 'debug.q' 'login.q' 'month_hdd.q' 'month_ssd.q' 'week_hdd.q' 'week_ssd.q' 'total'
        do
            for i in 3 4 5
            do
                grep "\s${queue}\s" parsed_data/gyoumu_phase1_2016${m}.tsv | \
                cut -f1,${i} | \
                dc_load.my -db ${DB[${i}]} -f TSV2 -ds account -p gyoumu_phase1_${queue}_2016${m} -op kv
            done
        done
    fi
done
