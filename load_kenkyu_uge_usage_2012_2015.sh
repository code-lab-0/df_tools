...
do
    ...
    do
        for p in 1 2
        do
            if [ -r parsed_data/kenkyu_phase${p}_${y}${m}.tsv ]; then
                ...
                do
                    for i in 3 4 5
                    do
                        grep "\s${queue}\s" parsed_data/kenkyu_phase${p}_${y}${m}.tsv | \
                        cut -f1,${i} | \
                        dc_load.my -db ${DB[${i}]} -f TSV2 -ds account -p kenkyu_phase${p}_${queue}_${y}${m} -op kv
                    done
                done
            fi
        done
